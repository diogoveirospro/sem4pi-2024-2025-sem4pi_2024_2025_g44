...
![US223 Class Diagram](images/class_diagram_us223.svg "US223 Class Diagram")

### 4.3. Applied Patterns

The system architecture continues to apply several recognized design patterns, ensuring a robust and maintainable structure. Below are the patterns applied to the functionality of listing customer representatives:
...
- **Description:** Perform conversion between domain entities and their respective DTOs. The transformation logic is isolated, promoting reuse and clarity in the application structure.

#### 3. **Repository Pattern**
- **Classes Involved:** `CustomerRepository`, `CustomerRepresentativeRepository`, `Repositories`
- **Description:** Abstract access to persistent data. They enable the retrieval of customers and their representatives through well-defined interfaces, decoupling the domain from the concrete persistence implementation.
...
- **Class Involved:** `EditCustomerRepresentativeController`
- **Description:** The controller manages interaction flows between the user interface and the domain. It orchestrates the listing of customers and their representatives, interacting with repositories and mappers to prepare the data for presentation.
...
- **Description:** Represent immutable values without individual identity, used to compose entities like `Customer` and `Representative`. They ensure expressiveness and consistency in the domain model.
